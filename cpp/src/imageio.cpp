...
{
    std::array<float, 3> ap = {static_cast<float>(a[0]), static_cast<float>(a[1]), static_cast<float>(a[2])};
    std::array<float, 3> bp = {static_cast<float>(b[0]), static_cast<float>(b[1]), static_cast<float>(b[2])};
...
std::array<uint8_t , 3> viridis_palette[] = {{68, 1, 84},    {72, 12, 115},  {70, 27, 126},  {64, 41, 129},
                                          {56, 54, 130},  {46, 66, 129},  {35, 78, 123},  {24, 91, 114},
                                          {15, 103, 103}, {17, 118, 90},  {36, 131, 76},  {68, 144, 63},
                                          {111, 155, 53}, {165, 166, 46}, {222, 176, 38}, {249, 210, 4}};
...
std::vector<uint8_t> apply_palette(const std::vector<uint8_t> &grayscale)
{
    ...
    for (auto i = 0u; i < grayscale.size(); ++i)
    {
        ...
        const auto color = lerp(viridis_palette[idx], viridis_palette[idx+1], static_cast<float>(alpha));
